...
export default function AdminPanel() {
  ...
  const addFlight = async (e: React.FormEvent) => {
    ...
    const res = await fetch('/api/admin/flights', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(flightForm) })
    const data = await res.json()
    setMessage(data.id ? '✅ Рейс добавлен!' : '❌ ' + (data.error || 'Ошибка'))
    if (data.id) {
      setFlightForm({ flightNumber: '', fromAirportId: 0, toAirportId: 0, departureTime: '', arrivalTime: '', startDate: '', endDate: '', daysOfWeek: '1,2,3,4,5,6,7', tariffLight: 4500, tariffStandard: 6500, tariffBusinessValue: 12500, tariffBusinessEasy: 15000 })
      fetchFlights()
    }
  }
  ...
  return (
    <div>
      ...
      {activeTab === 'flights' && (
        <div className="card">
          <h3>➕ Рейс</h3>
          <form onSubmit={addFlight} style={{ display: 'grid', gap: 15 }}>
