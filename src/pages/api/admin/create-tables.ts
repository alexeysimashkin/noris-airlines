...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const queries = [
    ...
    `CREATE TABLE IF NOT EXISTS "User" (id SERIAL PRIMARY KEY, "lastName" TEXT NOT NULL, "firstName" TEXT NOT NULL, "middleName" TEXT, "birthDate" TIMESTAMP NOT NULL, gender TEXT NOT NULL, email TEXT UNIQUE NOT NULL, phone TEXT NOT NULL, password TEXT NOT NULL, verified BOOLEAN DEFAULT false, "verifyToken" TEXT, "createdAt" TIMESTAMP DEFAULT NOW())`,
    `INSERT INTO "Aircraft" (id, type, model, "totalRows", "seatConfig") VALUES (1, 'Boeing 737-800', '738', 31, '{}') ON CONFLICT (id) DO NOTHING`,
    `ALTER TABLE "Booking" ADD COLUMN IF NOT EXISTS "userId" INTEGER`,
  ]
  ...
  try {
    ...
    res.json({ success: true, message: '✅ Все таблицы обновлены!', count: results.length })
  } catch (e: any) {
