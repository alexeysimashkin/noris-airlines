...
export default function AdminPanel() {
  ...
  const [bookingForm, setBookingForm] = useState({ flightId: 0, tariffId: 0, email: '', firstName: '', lastName: '', phone: '', seat: '', baggage: '', meal: '' })
  ...
  const handleEditFlight = (flight: Flight) => {
    ...
    setFlightForm({ flightNumber: flight.flightNumber, fromAirportId: fromAp?.id || 0, toAirportId: toAp?.id || 0, departureTime: flight.departureTime.slice(0, 16), arrivalTime: flight.arrivalTime.slice(0, 16), startDate: flight.startDate.slice(0, 10), endDate: flight.endDate.slice(0, 10), daysOfWeek: flight.daysOfWeek, tariffLight: flight.tariffs?.find(t => t.name === 'Лайт')?.price || 4500, tariffStandard: flight.tariffs?.find(t => t.name === 'Стандарт')?.price || 6500, tariffBusinessValue: flight.tariffs?.find(t => t.name === 'Бизнес Выгодно')?.price || 12500, tariffBusinessEasy: flight.tariffs?.find(t => t.name === 'Бизнес Легко')?.price || 15000 })
  ...
  const addBooking = async (e: React.FormEvent) => {
    ...
    if (!bookingForm.flightId || !bookingForm.tariffId || !bookingForm.email || !bookingForm.firstName || !bookingForm.lastName) {
      setMessage('❌ Заполните обязательные поля'); return
    }
    ...
    if (data.id) { setBookingForm({ flightId: 0, tariffId: 0, email: '', firstName: '', lastName: '', phone: '', seat: '', baggage: '', meal: '' }); fetchBookings() }
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
        <button className={`btn ${activeTab === 'airports' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setActiveTab('airports')}>🏢 Аэропорты</button>
        <button className={`btn ${activeTab === 'flights' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setActiveTab('flights')}>✈ Рейсы</button>
        <button className={`btn ${activeTab === 'meals' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setActiveTab('meals')}>🍽️ Питание</button>
        <button className={`btn ${activeTab === 'bookings' ? 'btn-primary' : 'btn-outline'}`} onClick={() => setActiveTab('bookings')}>📋 Бронирования</button>
      </div>
      ...
      {activeTab === 'airports' && (
        <div className="card">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 10 }}>
            {airports.map(a => <div key={a.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 14px', background: '#f5f3ff', borderRadius: 20, fontSize: 13 }}><strong>{a.iata}</strong> — {a.city} ({a.name}) <button onClick={() => deleteAirport(a.id)} style={{ cursor: 'pointer', background: 'none', border: 'none', color: 'red', fontSize: 16 }}>×</button></div>)}
          </div>
        </div>
      )}
      ...
      {activeTab === 'flights' && (
        <div className="card">
          ...
          <form onSubmit={handleFlightSubmit} style={{ display: 'grid', gap: 15 }}>
            ...
            <div className="form-group"><label className="form-label">Дни недели</label><input type="text" className="form-input" value={flightForm.daysOfWeek} onChange={e => setFlightForm({...flightForm, daysOfWeek: e.target.value})} /></div>
            <hr /><h4>💰 Тарифы (₽)</h4>
            <div className="grid grid-2">
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button type="submit" className="btn btn-primary">{editingFlightId ? '💾 Сохранить' : '➕ Добавить'}</button>
        ...
        <div className="card">
          ...
          <form onSubmit={addBooking} style={{ display: 'grid', gap: 15 }}>
            <div className="grid grid-2">
              <div className="form-group">
                ...
                <select className="form-select" value={bookingForm.flightId} onChange={e => { 
                  setBookingForm({...bookingForm, flightId: Number(e.target.value), tariffId: 0 })
                }} required>
              ...
              <div className="form-group">
                ...
                <select className="form-select" value={bookingForm.tariffId} onChange={e => setBookingForm({...bookingForm, tariffId: Number(e.target.value)})} required>
                  ...
                  {flights.find(f => f.id === bookingForm.flightId)?.tariffs?.map(t => <option key={t.id} value={t.id}>{t.name} — {t.price?.toLocaleString()} ₽</option>)}
                </select>
              </div>
            </div>
            <div className="grid grid-2">
            ...
            <div className="grid grid-2">
              <div className="form-group"><label className="form-label">Место (необязательно)</label><input type="text" className="form-input" placeholder="3A" value={bookingForm.seat} onChange={e => setBookingForm({...bookingForm, seat: e.target.value})} /></div>
              <div className="form-group"><label className="form-label">Питание (ID)</label><select className="form-select" value={bookingForm.meal} onChange={e => setBookingForm({...bookingForm, meal: e.target.value})}><option value="">Без питания</option>{meals.map(m => <option key={m.id} value={m.id}>{m.name} — {m.price}₽</option>)}</select></div>
            </div>
          ...
          {bookings.length === 0 ? <p style={{ color: '#999' }}>Нет бронирований</p> : (
            <table className="admin-table"><thead><tr><th>Код</th><th>Пассажир</th><th>Рейс</th><th>Тариф</th><th>Сумма</th><th>Статус</th></tr></thead><tbody>{bookings.map(b => (
              <tr key={b.id}><td><strong>{b.bookingCode}</strong></td><td>{b.passengers?.[0]?.passenger?.lastName} {b.passengers?.[0]?.passenger?.firstName}</td><td>{b.flight?.flightNumber} — {b.flight?.fromAirport?.city}→{b.flight?.toAirport?.city}</td><td>{b.tariff?.name}</td><td>{b.totalPrice?.toLocaleString()} ₽</td><td>{b.status === 'confirmed' ? '✅' : '↩'}</td></tr>
            ))}</tbody></table>
          )}
        </div>
      )}
